...
var range = function(x, y) {
  ...
  if (x > y) { //range (7,2) -> [6,5,4,3]
    let nums = range(x-1, y); //nums = range(6,2)
    nums.unshift(x-1); //nums = [6]
    return nums
  }
  else if (x < y)
  {
...
var exponent = function(base, exp) {
  ...
  return base * exponent(base, exp-1) // 8 * exponent (8, 1) 
  } else if (exp < 0) {
...
var reverse = function(string) {
    return string.length ? reverse(string.substr(1)) + string[0] : string;
}; //if string still has a length
//create a substring stasrting from index 1
//add it to the beginning of index 0 
//if string doesn't have a length
//return the string

// 10. Write a function that determines if a string is a palindrome.
var palindrome = function(string) {
  string = string.toLowerCase() //ignore lower case
  string = string.split(" ").join("") //ignore spaces
  if(string.length === 1){ //if you're down to 1 character, it's true!
    return true
...
var reverseArr = function (array) {
  if(!array.length){
    ...
    var strArray = reverseArr(array.splice(1));
    strArray.push(array[0]);
    return strArray
  }
...
var capitalizeWords = function(input) {
  if (input.length === 0) {
    ...
    var list = capitalizeWords(input.splice(1))
...
var capitalizeFirst = function(array) {
  if (array.length === 0) {
    ...
    var list = capitalizeFirst(array.splice(1))
...
var minimizeZeroes = function(array) {
  ...
  //recursion with a substring, removing one zero
